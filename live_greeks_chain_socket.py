...
# entirely client-side from ltp/spot/strike/expiry (2026-08-28) — so this UI
# chain no longer solves Black-Scholes on the backend at all;
# fetch_full_chain is always called with compute_greeks=False below and every
# row's iv/delta/gamma/theta/vega comes back zeroed. Real (non-display)
# Greeks are still computed backend-side wherever an actual trading decision
# needs them — see execution_socket.py / live_event.py / contracts.py and
# algo.scanner's historical chain snapshot, none of which go through this
# file.
